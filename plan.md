...
| VNC 탭(브라우저에 GUI 컨테이너 화면 라이브 임베드) | 구현 완료·실측 검증 완료(2026-08-25). **새 프록시 메커니즘이 아니라 App Routes 위에 얹힌 레지스트리** — router의 Caddy는 stock 빌드(HTTP/WS 전용, layer4 플러그인 없음)라 raw RFB를 태울 수 없으므로, 실제로 중계되는 건 대상이 wayvnc 앞에 띄운 웹 VNC 프런트엔드(noVNC+websockify, 보통 `:6080`)이고 그건 App Routes가 이미 맞는 운반 수단이다. 그래서 `internal/vnc`는 App Route가 개념적으로 모르는 것(표시 라벨, 뷰어 백엔드)만 자기 레지스트리(`/var/lib/code-docker-router/vnc/targets.json`)에 갖고 App Route를 lockstep으로 생성/수정/삭제한다. `List`는 자기가 쓴 걸 캐시하지 않고 매번 approutes를 다시 읽어 `routeMissing`/`routeDiverged`를 보고하고(App Routes 탭에서 손댄 변경이 조용히 묻히지 않도록), 없어진 route는 `Update`가 다시 만들어 self-heal한다. `Backend` 필드는 값이 `novnc` 하나뿐인데도 처음부터 존재 — Selkies 추가가 `backendViewer` 항목 한 줄로 끝나게 하려는 의도적 설계(원 계획의 "타겟별 선택 가능하게" 결정). 뷰어 iframe의 origin은 `window.location.origin`을 그냥 못 쓴다(`/app/`은 공유 호스트네임에만 서비스되고 `ROUTER_MANAGER_HOSTS` 전용 도메인엔 의도적으로 없음) — webmanager `RouterFrame`이 `?origin=`으로 알려주고, 전용 도메인을 직접 연 경우엔 뷰어 대신 이유를 설명한다(`useViewerOrigin.ts`). 자세한 내용: `docs/vnc.md`, 설계/조사 전 과정은 code-docker 쪽 `.claude/archive/router-vnc-tab-plan-done.md` |
...
- code-docker 쪽 `.claude/archive/router-vnc-tab-plan-done.md` — VNC 탭의 아이디어→
  리서치→결정→구현 전 과정. Selkies 전환과 `wayvnc --gpu` 하드웨어 인코딩이 트리거
  조건과 함께 백로그로 남아 있는 곳이기도 함
