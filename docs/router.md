...
사용자를 전부 지우면 tinyauth는 로그인시킬 대상이 없으므로 기동하지 않고 대기
상태로 들어갑니다(로그에 그 이유가 남습니다) — "인증 필요"로 설정한 expose는 그
동안 아무도 열 수 없습니다. 사용자를 다시 추가하면 그때 정상 기동합니다.

> **예전 버전에서 만든 사용자 복구.** 비밀번호 해시가 디스크에 기록되지 않던
> 버그(`users.json`에 이름만 저장됨)가 있었습니다. 그 시절에 만든 사용자는
> 로그인할 수 없고 tinyauth 설정에서도 제외되며, UI 목록에 "비밀번호 재설정 필요"
> 배지가 붙습니다 — "비밀번호 변경"으로 새 비밀번호를 지정하면 그대로 복구됩니다.
> (그 버그가 살아있던 동안에는 tinyauth가 `invalid user format`으로 즉시 종료돼서,
> 사용자 추가/삭제가 `supervisor fault 50: SPAWN_ERROR: tinyauth`로 실패했습니다.)
