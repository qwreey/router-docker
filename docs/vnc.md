...
VNC 탭은 터미널 탭처럼 화면을 꽉 채우고, 맨 위에 탭 바가 있습니다. 첫 탭
**대상 목록**은 항상 있고 닫히지 않으며, 대상 추가·편집·삭제와 연결된
클라이언트 목록이 여기 있습니다. 목록의 대상마다 여는 방법이 두 가지 있습니다.

- **열기** — 그 대상을 탭으로 엽니다. 여러 대상을 동시에 열어 두고 탭으로
  오갈 수 있습니다. 이미 열려 있으면 버튼이 **탭으로 이동**으로 바뀌고, 두 번째
  연결을 만들지 않고 그 탭을 보여줍니다. 탭 이름은 항상 대상에 설정된 이름
  (이름표가 없으면 대상 이름)이고, 뷰어에서 바꿀 수 없습니다.
- **새 창** — 뷰어를 독립된 창(탭)으로 엽니다. 탭으로 먼저 열 필요 없이
  목록에서 바로 누를 수 있습니다 — 전체 화면 크기로 보는 게 목적이라면
  대개 이쪽이 원하는 동작입니다.

탭의 연결은 이렇게 다룹니다.

- **탭을 닫으면(×) 연결이 끊깁니다.** 화면에는 "나중을 위해 켜 두기"가
  없습니다.
- **다른 탭을 보고 있는 동안에는 연결이 유지됩니다.** VNC 탭 안에서 다른
  대상 탭으로 가든, router(또는 webmanager)의 다른 메뉴로 가든 마찬가지입니다.
  숨은 뷰어도 크기는 그대로 유지해서, "원격 해상도" 대상이 탭을 오갈 때마다
  해상도를 바꾸지 않습니다. 대신 숨은 동안에도 화면 갱신은 계속 받습니다.
- **새로고침하면 열려 있던 탭과 보고 있던 탭이 복원됩니다**(브라우저마다,
  이 origin의 localStorage). 그사이 삭제된 대상의 탭은 조용히 빠집니다.
  `?target=<이름>`을 붙여 열면 그 대상을 탭으로 열어 보여줍니다.

code-docker의 code-server 확장 안에서는 탭 관리를 VS Code에 맡깁니다(`?mode=host`):
대상마다 에디터 탭이 하나씩 생기고, 그 안에는 탭 바 없이 그 대상의 화면만
보입니다. 대상 목록 뷰의 "열기"는 새 에디터 탭을 여는 요청이 됩니다.

이미 그 대상이 탭으로 열려 있으면 **새 창** 버튼은 **"새 창으로 옮기기"**로
바뀌고, 말 그대로 **옮깁니다** — 그 탭을 먼저 닫아 연결을 끊고, 그게 정리된
다음에 새 창이 붙습니다. 한 대상에 클라이언트 두 개가 동시에 붙어 있으면
"원격 해상도" 모드에서 서로 크기를 되돌리는데, 임베드 쪽이 대개 더 작아서 새로
연 큰 창이 자기 크기를 못 가져가기 때문입니다.

대상 탭을 보고 있으면 탭 바 오른쪽 끝에 버튼 두 개가 나옵니다.
...
- **전체화면** — 가능하면 **noVNC 자신의 전체화면 버튼을 대신 눌러줍니다.**
  ...
  버튼의 아이콘도 현재 상태를 따라갑니다.
  noVNC 쪽 버튼에 손이 닿지 않는 경우(전용 도메인을 쓰는 cross-origin 임베드
  등)에는 iframe이 아니라 **탭 바를 포함한 VNC 영역 전체**를 전체화면으로
  만듭니다 — 그러면 탭 바가 화면에 남아서 이 버튼으로 그대로 빠져나올 수 있습니다.
- **새 창으로 옮기기** — 위에서 설명한 핸드오프입니다. 전체화면이 정책상
  거부되는 환경에서의 대안이기도 합니다.
...
목록의 "연결" 열에 있는 배지가 그 대상에 지금 몇 개의 클라이언트가 붙어있는지
보여줍니다(5초마다 갱신, 탭이 백그라운드일 때는 멈춥니다). 배지를 누르면 IP,
브라우저/OS를 대충 짐작한 라벨, 접속한 지 얼마나 됐는지를 보여주는 목록이
펼쳐지고, 각 행의 "끊기"로 그 연결을 강제로 끊을 수 있습니다.

> ⚠️ **이 목록은 router의 `rfb` 중계를 거친 연결만 봅니다.** 대상의 raw RFB
> 포트에 직접 붙은 네이티브 VNC 클라이언트(예: Net 관리 탭의 Forwards로 노출한
> 포트를 통해 TigerVNC로 접속한 경우)는 router를 거치지 않으므로 이 목록에
> 전혀 나타나지 않고, 여기서 끊을 수도 없습니다. "여기 아무도 없다"가 "아무도
> 안 붙어있다"를 뜻하지 않습니다 — router가 볼 수 있는 것만 보여줍니다.
>
> IP도 router 앞단 nginx가 알려준 값을 그대로 보여주는 것뿐입니다 — router
> 앞에 클라이언트 IP를 넘겨주지 않는 또 다른 리버스 프록시를 뒀다면, 여기
> 보이는 IP는 실제 접속자가 아니라 그 프록시의 IP일 수 있습니다.

"끊기"는 그 클라이언트의 브라우저 탭 자체를 닫지 못합니다 — 소켓만 끊습니다.
noVNC 뷰어는 기본적으로 자동 재접속을 하므로, 단순히 연결을 끊기만 하면 그
브라우저가 몇 초 안에 다시 붙어서 아무 일도 없었던 것처럼 보일 수 있습니다.
그래서 "끊기"는 그 대상+IP+User-Agent 조합의 재접속을 짧은 시간(15초) 동안
거부합니다 — 그 사이 noVNC의 자동 재접속 시도는 거부되어 뷰어가 "연결 안 됨"
화면에 머무르고, 그 창을 실제로 쓰던 사람이 페이지를 새로고침하거나 다시
열면 그때는 정상적으로 다시 연결됩니다. 즉 이 기능은 "그 사람을 영구 차단"이
아니라 "지금 이 화면을 잠깐 꺼서 정리할 시간을 번다"에 가깝습니다.

IP만이 아니라 User-Agent까지 함께 보는 이유는, 같은 공유기/사내망 뒤의
서로 다른 두 브라우저가 같은 IP로 보이는 경우가 흔해서입니다 — IP만 봤다면
한 사람을 끊으려다 같은 네트워크의 다른 사람까지 15초간 재접속을 못 하게
막는 꼴이 됩니다. 다만 같은 브라우저에서 연 탭 두 개는 IP도 User-Agent도
같으므로 여전히 함께 끊깁니다 — 이건 의도된 동작입니다. 뷰어 헤더의
"새 창으로 옮기기"가 애초에 막으려는 "클라이언트 두 개가 desktop 크기를
두고 다투는" 상황이 정확히 이 경우이기 때문입니다.
