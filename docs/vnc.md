...
목록의 대상마다 여는 방법이 두 가지 있습니다.

- **보기** — 같은 페이지 아래에 뷰어를 엽니다.
- **새 창** — 뷰어를 독립된 창(탭)으로 엽니다. 뷰어를 먼저 열 필요 없이
  목록에서 바로 누를 수 있습니다 — 전체 화면 크기로 보는 게 목적이라면
  대개 이쪽이 원하는 동작입니다.

이미 그 대상의 뷰어가 아래에 열려 있으면 이 버튼은 **"새 창으로 옮기기"**로
바뀌고, 말 그대로 **옮깁니다** — 임베드된 뷰어를 먼저 닫아 연결을 끊고, 그게
정리된 다음에 새 창이 붙습니다. 한 대상에 클라이언트 두 개가 동시에 붙어 있으면
"원격 해상도" 모드에서 서로 크기를 되돌리는데, 임베드 쪽이 대개 더 작아서 새로
연 큰 창이 자기 크기를 못 가져가기 때문입니다. 뷰어 헤더의 같은 이름 버튼도
동일하게 동작합니다.
